...
interface RealDayViewManagerProps {
  userId: string;
  trackSlug: string;
  userScore: number;
  userProfile?: any;
...
const RealDayViewManager: React.FC<RealDayViewManagerProps> = ({
  userId,
  trackSlug,
  userScore,
  userProfile,
  ...
  const trackInfo = {
    ...
    equilibrio: {
      ...
      subtitle: 'Redescobrindo o controle',
      duration: 21,
      color: '#F59E0B',
      icon: Target,
    },
  ...
  const loadDayContent = async (dayNumber: number) => {
    try {
      setLoading(true);
      const content = await PersonalizationService.getPersonalizedContent(userId, trackSlug, dayNumber);
  ...
  const getActivityIcon = (type: string) => {
    switch (type) {
      case 'devotional': return <Heart className="w-5 h-5" />;
  ...
  const getTypeColor = (type: string) => {
    switch (type) {
      case 'devotional': return 'text-purple-600 bg-purple-100';
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2">
            <Card className="overflow-hidden">
              ...
              {/* Devotional Content - SEMPRE PRESENTE */}
              {dayContent?.devotionalContent && (
                <div className="px-8 py-6 bg-gradient-to-r from-purple-50 to-indigo-50 border-b">
                  <h3 className="text-lg font-semibold mb-4 text-purple-900">🙏 Momento Devocional</h3>
                  
                  <div className="space-y-4">
                    <div className="bg-white p-4 rounded-lg border border-purple-200">
                      <h4 className="font-medium text-purple-800 mb-2">Versículo do Dia</h4>
                      <p className="text-gray-700 italic">"{dayContent.devotionalContent.verse}"</p>
                    </div>
                    
                    <div className="bg-white p-4 rounded-lg border border-purple-200">
                      <h4 className="font-medium text-purple-800 mb-2">Reflexão</h4>
                      <p className="text-gray-700">{dayContent.devotionalContent.reflection}</p>
                    </div>
                    
                    <div className="bg-white p-4 rounded-lg border border-purple-200">
                      <h4 className="font-medium text-purple-800 mb-2">Oração</h4>
                      <p className="text-gray-700">{dayContent.devotionalContent.prayer}</p>
                    </div>

                    {dayContent.devotionalContent.audioUrl && (
                      <AudioPlayer
                        src={dayContent.devotionalContent.audioUrl}
              ...
              <CardContent className="p-8">
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex items-center space-x-4">
                            <div className={`p-2 rounded-lg ${
                              activity.completed ? 'bg-green-500 text-white' : getTypeColor(activity.type)
                            }`}>
                ...
                {dayContent && (
                  <div className="mt-8 p-6 bg-gray-50 rounded-xl">
                    <div className="flex items-center justify-between">
                      <div>
                        <h4 className="font-semibold text-gray-900">Completar Dia</h4>
                        <p className="text-sm text-gray-600 mt-1">
                          Complete todas as atividades obrigatórias para finalizar o dia
                        </p>
                      </div>
                      
                      <Button 
                        onClick={completeDayProgress}
                        disabled={!allRequiredActivitiesCompleted()}
                        size="lg"
                        className="min-w-[120px]"
                      >
                        {allRequiredActivitiesCompleted() ? 'Finalizar Dia' : 'Em Progresso'}
                      </Button>
                    </div>
                    
                    {dayContent.rewards.achievement && (
                      <div className="mt-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
                        <div className="flex items-center">
                          <Trophy className="w-5 h-5 text-yellow-600 mr-2" />
                          <span className="text-sm font-medium text-yellow-800">
                            Conquista disponível: {dayContent.rewards.achievement}
                          </span>
                        </div>
                      </div>
                    )}
...
export default RealDayViewManager;
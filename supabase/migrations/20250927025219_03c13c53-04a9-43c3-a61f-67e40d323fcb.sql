-- PASSO 1: Remover TODAS as políticas existentes das tabelas problemáticas
DROP POLICY IF EXISTS "Enable all for admin only" ON public.track_content;
DROP POLICY IF EXISTS "Super admins can manage track content" ON public.track_content;
...
DROP POLICY IF EXISTS "Usuários veem apenas suas próprias roles" ON public.user_roles;

-- PASSO 2: Remover funções que causam recursão infinita
...
-- PASSO 3: Criar funções de segurança simples e não-recursivas
...
-- PASSO 4: Recriar função get_user_role sem recursão
...
-- PASSO 5: Criar políticas RLS simples para track_content
CREATE POLICY "Admins manage track content"
...
-- PASSO 6: Criar políticas RLS simples para admin_users
...
-- PASSO 7: Criar políticas RLS simples para user_roles
...
-- PASSO 8: Criar política para admin_activity_logs

...
create policy if not exists "Users can view own results"
  on public.questionnaire_results
  for select
  using (auth.uid() = user_id);

create policy if not exists "Users can insert own results"
  on public.questionnaire_results
  for insert
  with check (auth.uid() = user_id);

create policy if not exists "Users can update own results"
  on public.questionnaire_results
  for update
  using (auth.uid() = user_id);

create policy if not exists "Users can delete own results"
  on public.questionnaire_results
  for delete
  using (auth.uid() = user_id);
